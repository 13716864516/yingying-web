...
function getUserInfo($uid, $type = 0)
{
    ...
    if (!$info) {
        $info = Db::name('users')
            ->field('id,user_nickname,avatar,avatar_thumb,sex,signature,birthday,type,signoryid,identity,father_mobile,mother_mobile,student_number')
            ->where(['id' => $uid])
            ->find();
        if ($info) {
            $info['age'] = '';
            if ($info['birthday']) {
                $days = (time()-$info['birthday'])/86400;
                $info['age'] = ((int)($days/365)).'岁'.((int)(($days%365)/30)).'个月';
            }
        } else if ($type == 1) {
            return $info;
